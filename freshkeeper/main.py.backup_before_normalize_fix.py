...
def list_grouped_lots():
    """
    Regroupe les lots par produit et trie chaque sous-lot par d̩lai restant (petit -> grand).
    ...
    for k, arr in groups.items():
        arr.sort(
            key=lambda x: (999999 if x["delai_restant"] is None else x["delai_restant"])
        )
...
def aa_get():
    # retourne la liste AA courante (ajouts libres c̫t̩ utilisateur)
