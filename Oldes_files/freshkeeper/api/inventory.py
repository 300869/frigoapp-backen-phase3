...
from freshkeeper.models.inventory import InventoryItem
from freshkeeper.models.product import Product
from freshkeeper.schemas.inventory import InventoryCreate, InventoryRead, InventoryUpdate

router = APIRouter()


# Helper: compute expires_at if missing
async def _compute_expiry(
    session: AsyncSession, product_id: int, provided: date | None
) -> date | None:
    if provided:
        return provided
...
async def list_items(
    expired: bool | None = Query(
        default=None, description="true=seulement expirés, false=seulement non expirés"
    ),
    expiring_in_days: int | None = Query(
        default=None, ge=1, description="Afficher ceux qui expirent dans N jours"
    ),
    current=Depends(get_current_user),
    session: AsyncSession = Depends(get_session),
):
    ...
    if expired is not None:
        ...
        if expired:
            stmt = stmt.where(
                InventoryItem.expires_at.is_not(None), InventoryItem.expires_at < today
            )
        else:
            stmt = stmt.where(
    ...
    if expiring_in_days is not None:
        ...
        stmt = stmt.where(
            InventoryItem.expires_at.is_not(None), InventoryItem.expires_at <= limit
        )
